...
const Taskdetails: React.FC = () => {
	...
	useEffect(() => {
		...
		dispatch(
			setActivity(
				tasks.find(task => task._id === params.taskId)
					?.activities as any
			)
		);
	}, [tasks]);
	...
	return (
		<>
			{accpetFormOpen && (
				<Dialog open={true}>
					...
					<DialogContent className='top-1/2 left-1/2 z-20 fixed bg-background shadow-lg p-4 rounded-lg w-[80vw] lg:w-full max-w-3xl transform -translate-x-1/2 -translate-y-1/2'>
						...
						<AcceptTaskForm
							assignee={taskData.assinees[0]._id}
          ...
          <Activites />
				...
				<Activites
					activityData={taskData.activities}
					taskId={params.taskId}
				/>
			</div>
		</>

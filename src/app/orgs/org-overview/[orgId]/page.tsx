...
const OrgOverview = () => {
  ...
  if (error) return <ErrorDisplay errorMessage={error.message || orgError?.message || taskerror?.message || 'Unknown error occurred'} />; 
  return (
    <div className="p-4">
          ...
          <div className="grid grid-cols-4 h-full">
            <div className="col-span-3">
              <div className="grid grid-rows-3 h-full">
                <div className="px-8 py-4">
                  <h1 className="mb-2 text-slate-700 text-xl">Spaces</h1>
                  <div className="flex flex-wrap gap-2 mt-4 overflow-hidden">
                    {spaces.map((spaceItem, index) => {
                      return (
                        <div
                          key={index}
                          className="border-2 border-slate-300 bg-secondary shadow p-2 rounded-md min-w-[260px] max-w-[300px] h-[100px]"
                        >
                          ...
                          <div className="flex justify-between mt-6 space-x-3 text-slate-500">
                ...
                <div className="mr-10 px-8 py-4 border-t-2">
                  <h1 className="mb-2 text-slate-700 text-xl">Open Tasks</h1>
                  {taskdata && taskdata.listAllTasks.length > 0 ? (
                    <ul className="flex flex-col flex-wrap gap-4 mt-6 overflow-hidden">
                      {taskdata?.listAllTasks.slice(0,4).map((opentask, index) => (
                        <li
                          key={index}
                    ...
                    <p className="mt-4 text-slate-500">This organization doesn't have any open tasks at the moment</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4 py-4 pr-20">
              ...
              <div className="flex flex-col gap-4 mt-4">
                <h1 className="text-slate-700 text-xl">Contributers</h1>

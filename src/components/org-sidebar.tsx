...
const OrgSidebar = ({ Title }: { Title: string }) => {
	...
	return (
		<aside className='col-span-1 bg-background border-r h-screen'>
			...
			<Spaces
				spacesItem={spaces as { name: string; visibility: boolean }[]}
			/>
			<SpacesAddModal />
		</aside>
	);
...
export const Spaces = ({
	...
	return (
		<div className='flex flex-col items-start mt-auto p-2'>
			<div className='flex justify-between items-center p-3.5 w-full'>
						...
						<li>
							<Button
								className={cn(
									...
									isPathMatch(currentURl, name) ||
										currentURl.startsWith(name)
										? 'text-primary-foreground bg-primary hover:text-primary-foreground hover:bg-primary'
										: ''
								)}
								asChild>

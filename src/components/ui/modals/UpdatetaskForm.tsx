...
const assineesOptions = [
  { value: "6672dba833963a34ca6b6b9d", label: "ak@gmail.com", icon: <Users className="w-4 h-4" /> },
  { value: "6672dba833963a34ca6b6b9d", label: "dayal@gmail.com", icon: <Users className="w-4 h-4" /> },
  { value: "6672dba833963a34ca6b6b9d", label: "vineet@gmail.com", icon: <Users className="w-4 h-4" /> },
];
...
const customPriorityOption = (props: any) => (
  <components.Option {...props}>
    <div className="flex items-center">
      <span className={`mr-2 h-2 w-2 rounded-full bg-${props.data.color}-500`} />
...
const UpdateTaskForm = ({
  ...
  const { callMethod } = useSmartContract(web3);

  const onSubmitForm = async (data: Schema) => {
    try {
      const result = await callMethod('UpdateTask', ['Error Handling', 100000000000000000, "0x45f520587bf5CA91c922dEFBc596A6A5Ce294039"]);
    ...
    <form autoComplete="off" onSubmit={handleSubmit(onSubmitForm)}>
      <div className="grid grid-cols-3 gap-6 p-4">
        <div className="col-span-2">
          ...
          <div className="mt-4">
            <Label className="text-md text-slate-800">Acceptance Criteria</Label>
            <Textarea
            ...
            {errors.acceptanceCriteria && (
              <span className="text-red-500 text-xs">{errors.acceptanceCriteria.message}</span>
            )}
          </div>

          <Button type="submit" className="block w-full bg-[#7D6CE2FF] text-center mt-4">
            Update
          </Button>
        </div>
        ...
        <div>
          <div className="flex flex-col justify-between items-center gap-4">
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    options={status}
                    defaultValue={status.find((s) => s.value === parseInt(column))}
            ...
            <div className="text-sm text-slate-900 uppercase w-full">
              <Label className="text-sm text-slate-800">assinees</Label>
              <Controller
                name="assinees"
                control={control}
                render={({ field }) => (
                  <Select
                    ...
                    components={{ Option: customOptionassinees, SingleValue: customSingleValueassinees }}
                  />
                )}
              />
              {errors.assinees && (
                <span className="text-red-500 text-xs">{errors.assinees.message}</span>
              )}
            </div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    components={{ Option: customPriorityOption, SingleValue: customPrioritySingleValue }}
                  />
                )}
              />
              {errors.priority && (
            ...
            <div className="text-sm text-slate-900 uppercase w-full">
              ...
              />
              {errors.reviewer && (
                <span className="text-red-500 text-xs">{errors.reviewer.message}</span>
              )}
            </div>
          </div>

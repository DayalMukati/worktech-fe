...
const CreateTaskForm = ({
	...
	const randomUser = () => {
		...
		return randomUserIndex !== -1 ? users[randomUserIndex] : null;
	};
	const randomAssignee = randomUser();
	const randomReviewer = randomUser();
	const Assignee = [
		{
			label: 'Recommended by AI 💡',
			options: [
				{
					value: [randomAssignee._id, randomAssignee.walletAddress],
					label: randomAssignee.email,
	...
	const Reviewers = [
		{
			...
			options: [
				{
					value: [randomReviewer._id, randomReviewer.walletAddress],
					label: randomReviewer.email,
					icon: <Bot className='w-4 h-4 text-primary' />
				}
			]
		},
